...
function App() {
    ...
    const findLongestWorkingPairs = () => {
                ...
                projectsList.map((empObj2) => {
                    if (empObj1.EmpID !== empObj2.EmpID) {
                        ...
                        if (!processedPairs.has(pairKey)) {
                            ...
                            pairs.map((pair1) => {
                                const objectWithKeyValue = longestPairs.find(obj => obj['employees'] && JSON.stringify(obj['employees']) === JSON.stringify(pair1.employees));
                                if (objectWithKeyValue) {
                                    const pairIndex = longestPairs.findIndex(pair => JSON.stringify(pair.employees) == JSON.stringify(pair1.employees));
                                    longestPairs[pairIndex].duration += pair1.duration;
        ...
        const sortedPairs = longestPairs.slice().sort((a, b) => b.duration - a.duration);
        const maxDuration = sortedPairs.length > 0 ? sortedPairs[0].duration : 0;
        let filteredPairs = sortedPairs.filter(item => item.duration === maxDuration);
        ...
        filteredPairs.map((item) => {
            ...
            projectsList.map(({ duration, project }) => {
                if (projectDurationsMap.has(project)) {
                    projectDurationsMap.set(project, projectDurationsMap.get(project) + duration);
                } else {
        ...
        const resultArray = filteredPairs.map((item) => {
            const { projectsList } = item;
            const updatedProjectsList = projectsList.map(({ duration, project }) => {
        ...
        resultArray.map((result) => {
            console.log(`${result.employees[0]}, ${result.employees[1]}, ${result.duration} days`);
            result.projectsList.map(el => console.log(`${el.project}, ${el.duration}`));
        })
    ...
    return (
        <div className="App">
            ...
            <div className="input-container">
                ...
                <button onClick={findLongestWorkingPairs}>Find Pairs</button>
            </div>
        </div>
    );
}
